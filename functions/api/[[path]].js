...
const sysConfig = reactive({
  ...
  notice_text: '欢迎使用', show_all_posts: true, passenger_fee: 0, driver_fee: 0, driver_cert_required: false,
...
onMounted(async () => {
  ...
  try {
    ...
    // 2. ★★★ 直达链接核心修复 ★★★
    // 优先检测 URL 参数，一旦有 ride_id，立即请求，不等待其他
...
// ===================== 核心：导航与返回 (V75 修正版) =====================
// 切换 Tab (不使用 Hash，直接改变量)
const switchTab = (idx) => { 
    if (activeTab.value === idx) return;

    // 从首页进入子页 -> 压入历史，这样按返回键能退回首页
...
const handlePopState = () => {
    ...
    if (activeTab.value === 0) {
        if (exitCounter === 0) {
            showToast('再按一次退出');
            exitCounter++;
            window.history.pushState(null, null, document.URL); // 锁住
