...
export async function onRequest(context) {
    ...
    try {
        ...
        if (url.pathname === '/api/rides' && method === 'POST') {
            ...
            // 1. 先查该 ID 对应的手机号
            const currentUser = await env.DB.prepare('SELECT phone FROM users WHERE id = ?').bind(data.user_id).first();
            if (!currentUser || !currentUser.phone) return jsonResponse({ error: '请先绑定手机号' }, 403);

            // 2. ★关键：查这个手机号是否被封 (status=0)
            // 哪怕换了 UserID，只要手机号一样，就看那个手机号下有没有被封的记录
            const bannedInfo = await env.DB.prepare('SELECT id FROM users WHERE phone = ? AND status = 0').bind(currentUser.phone).first();
            
            if (bannedInfo) return jsonResponse({ error: '该手机号已被封禁，无法发布' }, 403);
