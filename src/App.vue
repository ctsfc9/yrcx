...
const getCarClass = (model) => {
  if (!model) return '';
  if (model.includes('混合')) return 'hybrid'; // 紫色
  if (model.includes('电')) return 'electric'; // 绿色
  return 'gas'; // 红色
};
...
const switchTab = (idx) => {
  ...
  if (idx === 0) { 
    ...
    // 进入发布或我的，push一条记录，确保按返回键是回到首页
    window.history.pushState({ page: 'sub' }, null, document.URL);
...
const openDetail = (item) => {
  // 打开详情页时，push一条记录，这样用户按返回键就会触发 popstate
...
// 点击UI上的“返回”按钮
const closeDetail = () => {
  // 手动调用 back，这会触发 popstate 从而关闭弹窗，且保持历史记录清洁
  window.history.back();
};
</script>
