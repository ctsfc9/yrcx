<script setup>
import { ref, reactive, computed, nextTick, onMounted, onUnmounted, onErrorCaptured } from 'vue';
import { showToast, showSuccessToast, showFailToast, showDialog, showLoadingToast } from 'vant';
...
// 捕捉任何 Vue 内部错误，显示在屏幕上而不是白屏
onErrorCaptured((err) => {
  console.error("Vue Error:", err);
  globalError.value = "界面渲染错误: " + err.message;
  return false;
});
...
const safeList = computed(() => {
  ...
  // 浅拷贝后再排序，防止修改原数组导致循环更新
  return [...list.value].sort((a, b) => new Date(a.date) - new Date(b.date));
});
...
const closeDetail = () => {
  // 触发一次后退，popstate 会处理关闭逻辑
  window.history.back();
};

const handlePopState = () => {
  // 1. 如果当前 hash 是空的，说明从 #detail 退回来了 -> 关闭弹窗
...
const initMapPicker = () => { if(window.AMap && !mapInstance) { nextTick(()=>{ /* init map */ }); } };
...
<template>
  <div v-if="globalError" style="position:fixed;top:0;left:0;width:100%;background:#ee0a24;color:#fff;z-index:99999;padding:15px;font-size:14px;text-align:center;">
    ⚠️ {{ globalError }} <br><span style="font-size:12px;opacity:0.8">请检查 main.js 是否引入了 vant 样式</span>
  </div>
  ...
  <div class="app-container">
    ...
    <div v-if="isSystemAdmin" class="admin-wrapper">
      ...
      <div v-else class="admin-dashboard">
        ...
        <div class="admin-main">
          <div v-if="adminActiveMenu==='basic'"><van-button @click="saveSystemConfig">保存</van-button></div>
    ...
    <div v-else class="user-wrapper">
      ...
      <div v-if="activeTab === 2" class="page-me">
        ...
        <div class="me-menu-grid"><van-grid :column-num="3"><van-grid-item icon="service-o" text="客服" @click="showDialog({message:'微信: '+sysConfig.kefu_wechat})"/></van-grid></div>
