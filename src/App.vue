...
onMounted(async () => {
  ...
  try {
    ...
    console.error("Non-fatal Init Error", e);
  }
});
...
const handlePopState = () => {
  ...
  // 4. 退出逻辑
  exitCounter++;
  if (exitCounter < 2) showToast('再按一次退出');
  setTimeout(() => { exitCounter = 0; }, 2000);
};
</script>

<template>
  <div v-if="globalError" style="position:fixed;top:0;left:0;width:100%;background:red;color:#fff;z-index:99999;padding:10px;">{{globalError}}</div>
