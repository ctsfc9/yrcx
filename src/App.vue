...
const exitClickCount = ref(0);    // 退出计数器
let exitTimer = null;             // 退出计数重置定时器
...
// 统一的页面跳转/打开方法
const pushHistoryState = (pageName) => {
  // 向浏览器历史栈推入一个状态
...
const handlePopState = (event) => {
  ...
  // 3. 如果在非首页Tab (发布页/个人中心) -> 回到首页
  if (activeTab.value !== 0) {
    activeTab.value = 0;
    return;
  }

  // 4. 如果在首页 -> 处理“连续按3次退出”逻辑
  // 注意：因为用户按了返回键，浏览器实际上已经后退了一步。
  // 为了防止页面真的退出，我们需要再次推入一个状态，把用户“按”回来。
  ...
  exitTimer = setTimeout(() => {
    exitClickCount.value = 0; // 2秒没按就重置
  }, 2000);

  if (exitClickCount.value >= 3) {
    // 达到3次，关闭微信窗口
    wx.closeWindow();
  } else {
    showToast(`再按 ${3 - exitClickCount.value} 次退出平台`);
  }
};
...
const updateWxShare = (customData = null) => {
  ...
  // 这里仅配置逻辑，实际需要后端签名支持 wx.config
  // 我们通过模拟设置，确保留给“引导遮罩”的文案是正确的
  // 在真实环境中，wx.ready 会自动读取最新的配置
  ...
  // 2. 配置分享到朋友圈 (通常只取 title)
  wx.onMenuShareTimeline({
    title: shareData.title + ' ' + shareData.desc, // 朋友圈技巧：把描述拼到标题里
    link: shareData.link,
    imgUrl: shareData.imgUrl,
    success: function () { showSuccessToast('分享成功'); }
  });
...
const initWxShare = () => {
  try {
    wx.ready(function () { updateWxShare(); });
...
const getDetailShareData = (item) => {
  ...
  const routeStr = `${item.origin}→${item.destination}`;
  const timeStr = formatDate(item.date);
  
  // 描述中不包含电话，但必须包含广告语
  const cleanRemark = (item.remark || '无备注');
  const desc = `出发:${timeStr}。备注:${cleanRemark}。${DEFAULT_SHARE.desc}`; // 强制拼接广告语
  
  return {
    title: `${typeStr} ${routeStr}`,
    desc: desc,
    link: DEFAULT_SHARE.link, // 依然跳回首页，因为没有做详情页独立路由
...
const openDetail = (item) => {
  ...
  pushHistoryState('detail'); // 记录历史
  
  // 更新分享信息
  const shareData = getDetailShareData(item);
  updateWxShare(shareData);
};
const closeDetail = () => {
  // 如果是手动点击关闭，也要模拟一次返回，保证历史记录堆栈平衡
  // 这里直接由 popstate 处理逻辑，手动点击时我们可以调用 history.back()
  window.history.back();
};
...
const openSubPage = (pageName) => { 
  ...
  pushHistoryState(pageName); // 记录历史
};
const closeSubPage = () => { 
