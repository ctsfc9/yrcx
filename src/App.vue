...
const showDatePicker = ref(false); // 时间选择器
const showSeatPicker = ref(false); // 座位选择器 (修复)
...
// 座位选项 (修复：格式化为 Vant Picker 需要的格式)
...
const onConfirmDate = ({ selectedOptions }) => {
  const dayOffset = selectedOptions[0].value;
  const hour = selectedOptions[1].value;
  const minute = selectedOptions[2].value;
  ...
  const dayName = selectedOptions[0].text; // 今天/明天
  
  // 存入表单用于显示
  postForm.dateDisplay = `${dayName} (${dateStr}) ${hour}:${minute}`;
  // 存入 ISO 格式用于后端
  const year = targetDate.getFullYear();
  const isoMonth = String(m).padStart(2, '0');
  const isoDay = String(d).padStart(2, '0');
  const isoHour = String(hour).padStart(2, '0');
  postForm.date = `${year}-${isoMonth}-${isoDay}T${isoHour}:${minute}`;
...
const handleRealPublish = async () => {
  ...
  try {
    ...
    const payload = { ...postForm, remark: remarkStr, pay_amount: 0, is_top: isTop.value ? 1 : 0, user_id: userProfile.id };
