...
<template>
  <div v-if="!appReady" style="display:flex;justify-content:center;align-items:center;height:100vh;background:#fff;">
    <van-loading size="24px" vertical color="#1989fa">正在加载...</van-loading>
  </div>
          ...
          <van-list v-model:loading="loading" :finished="finished" finished-text="无更多">
            <div v-for="item in safeList" :key="item?.id || Math.random()" class="ride-card" @click="selectedRide = item">
              ...
              <div class="card-row-2">
                <div class="info-item"><van-icon name="clock-o" color="#666"/> {{ formatDate(item.date) }}</div>
