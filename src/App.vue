<script setup>
import { ref, reactive, computed, onMounted, onErrorCaptured, nextTick, onUnmounted } from 'vue';
...
// ==========================================
const debugMsg = ref('');
const appReady = ref(true); // 默认强制为 true，绝不隐藏

// 捕获 Vue 内部错误
onErrorCaptured((err) => {
  debugMsg.value = `Vue Error: ${err.message}`;
  return false;
});

// 捕获全局 JS 错误
window.onerror = function(message, source, lineno, colno, error) {
  debugMsg.value = `System Error: ${message} (Line: ${lineno})`;
};
...
onMounted(async () => {
  try {
    ...
    // 异步加载，即使失败也不影响显示
    fetchSystemConfig().catch(e => console.warn('Config Error', e));
    onLoad().catch(e => console.warn('Load Error', e));
    ...
    debugMsg.value = "Init Error: " + e.message;
  }
});
...
const initMapPicker = () => { if(window.AMap && !mapInstance) { nextTick(()=>{ /* init */ }); } };
...
const handleUserDelete = (id) => { showDialog({title:'提示',message:'确认删除?'}).then(async ()=>{ await fetch(`/api/rides?id=${id}&user_id=${userProfile.id}`, { method: 'DELETE' }); onLoad(); }); };
</script>

<template>
  <div v-if="debugMsg" style="position:fixed;top:0;left:0;right:0;background:red;color:#fff;z-index:99999;padding:10px;font-size:12px;word-break:break-all;">
    {{ debugMsg }}
  </div>
    ...
    <div v-if="isSystemAdmin" class="admin-wrapper">
      ...
      <div v-else class="admin-dashboard">
        ...
        <div class="admin-main">
          ...
          <div v-else><van-button @click="saveSystemConfig">保存</van-button></div>
        </div>
      </div>
    </div>
