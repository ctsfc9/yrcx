...
const sysConfig = reactive({
  ...
  tags_passenger: '有行李,走高速,只限女生,已有3人',
  banners: '',
  amap_key: '',
  sms_account: '',
  sms_password: ''
...
const saveSystemConfig = async () => {
  ...
  const res = await fetch('/api/admin?action=save_config', {
    method: 'POST',
    body: JSON.stringify({ auth_token: adminLoginData.password, config: sysConfig })
  });
...
const fetchAdminRides = async () => {
  const res = await fetch(`/api/admin?action=get_rides&token=${adminLoginData.password}`);
  const data = await res.json();
...
const deleteRideAdmin = (id) => {
  showDialog({ title: '删除', message: '确定删除？', showCancelButton: true }).then(async (action) => {
    if (action === 'confirm') {
      await fetch('/api/admin?action=manage_ride', { 
        method: 'POST', 
...
const banUserAdmin = (userId, isBan) => {
  showDialog({ title: isBan?'封禁':'解封', message: `确定操作用户 ${userId}？`, showCancelButton: true }).then(async (action) => {
    if (action === 'confirm') {
      await fetch('/api/admin?action=manage_user', { 
        method: 'POST', 
...
const fetchSystemConfig = async () => {
  try {
    ...
    const data = await res.json();
    if(Object.keys(data).length > 0) Object.assign(sysConfig, data);
    document.title = sysConfig.platform_name;
  } catch(e) {}
};
...
const openDetail = (item) => { selectedRide.value = item; updateWxShare({title:`${item.origin}→${item.destination}`, desc:`出发:${formatDate(item.date)}`}); window.history.pushState({page:'detail'},''); };
...
<template>
  <div v-if="isSystemAdmin" class="admin-layout">
    <div v-if="!isLogined" class="admin-login-box">
      <h3>{{ sysConfig.platform_name }} - 后台</h3>
    ...
    <div v-else class="admin-dashboard">
      <div class="admin-sidebar">
        ...
        <div class="menu-item logout" @click="()=>location.href='/'">返回前台</div>
      </div>
          ...
          <van-form @submit="saveSystemConfig">
            <van-tabs>
              <van-tab title="基本信息">
                ...
                <van-field v-model="sysConfig.platform_desc" label="描述" type="textarea" />
              ...
              <van-tab title="费用&开关">
                ...
                <van-field v-model="sysConfig.publish_fee_passenger" label="乘客费用" type="number" />
                <van-field v-model="sysConfig.publish_fee_driver" label="司机费用" type="number" />
        ...
        <div v-if="adminActiveMenu==='users'">
          <div class="admin-list">
            <div v-for="u in adminUserList" :key="u.user_id" class="admin-list-item">
              <span>{{ u.user_id }} (贴:{{ u.post_count }})</span>
  ...
  <div v-else class="app-container">
    <div v-if="activeTab === 1" class="page-post new-post-style">
      ...
      <div class="post-card">
        ...
        <div class="tags-group"><van-checkbox-group v-model="postForm.remark" direction="horizontal"><van-checkbox v-for="t in currentRemarkOptions" :key="t" :name="t" shape="square" class="tag-item">{{t}}</van-checkbox></van-checkbox-group></div>
      </div>
    ...
    <div v-if="activeTab === 2" class="page-me">
      <div class="user-card"><div class="user-header"><div class="avatar-circle">头</div><div class="user-info"><div class="nickname">{{ userProfile.nickname }}</div></div></div></div>
      <div class="me-menu-grid"><van-grid :column-num="3" clickable>
        ...
        <van-grid-item icon="manager-o" text="后台管理" @click="()=>location.href='/admin'" style="color:#ee0a24" />
      </van-grid></div>
    </div>
