...
const autoLocate = () => {
  ...
  AMap.plugin('AMap.Geolocation', function() {
    ...
    geolocation.getCurrentPosition(function(status, result) {
      if(status == 'complete'){
        // ★★★ 隐私修复：仅保留区/县/街道，去除详细门牌 ★★★
        const ac = result.addressComponent;
        if (ac) {
          // 拼接格式：区/县 + 街道/乡镇
          let safeAddr = (ac.district||'') + (ac.township||'') + (ac.street||'');
          if (!safeAddr) safeAddr = result.formattedAddress; // 降级
          postForm.origin = safeAddr;
          showSuccessToast('定位成功');
        } else {
          postForm.origin = result.formattedAddress;
          showSuccessToast('已获取位置');
        }
        closeToast();
      } else {
        closeToast();
...
const checkPriceLimit = () => {
  if (postForm.price > 9999) {
    postForm.price = 9999;
    showToast('费用上限 9999 元');
  }
};
  ...
  <div v-else>
    ...
    <div v-else class="app-container">
      <div v-if="activeTab === 1" class="page-post new-post-style">
        <van-nav-bar title="发布" left-arrow @click-left="switchTab(0)" />
        <div class="post-card">
          ...
          <div class="info-group">
            ...
            <div class="form-row"><div class="label">费用</div><div style="flex:1"><van-field v-model="postForm.price" type="number" placeholder="元" input-align="right" @input="checkPriceLimit"/></div></div>
      ...
      <van-popup v-model:show="showMapPopup" position="bottom" :style="{height:'90%'}" round><div class="map-popup-content"><van-search v-model="mapSearchKeyword" show-action placeholder="搜索" @search="onMapSearch"><template #action><div @click="showMapPopup=false">关闭</div></template></van-search><van-list><van-cell v-for="(i,k) in mapSearchResults" :key="k" :title="i.name" @click="selectLocation(i)"/></van-list></div></van-popup>
