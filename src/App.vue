...
onMounted(async () => {
  ...
  // 2. 首页防退出守卫
  window.history.pushState({ page: 'guard' }, null, window.location.href);
...
const loadAMapScript = (key) => { 
    ...
    try{ 
        ...
        s.src=`https://webapi.amap.com/maps?v=2.0&key=${key}&plugin=AMap.CitySearch,AMap.AutoComplete,AMap.Geocoder`; 
        s.onload = () => { 
