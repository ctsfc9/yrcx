...
const handlePopState = () => {
    ...
    if (uiState.selectedRide) {
        ...
        else window.history.pushState(null, null, document.URL); // 补回
        return;
    }

    // 3. ★★★ 修复：二级页面 -> 回首页 ★★★
    // 只有当 activeTab 不为 0 时，我们才认为是“退回首页”
    if (activeTab.value !== 0) {
        activeTab.value = 0;
        // 注意：这里不需要 pushState，因为浏览器已经执行了“后退”，URL 已经变回去了(或者只是栈退了)
        // 我们只需要响应这个变化，把视图切回首页即可
        return;
    }
...
const switchTab = (idx) => { 
    ...
    // 如果是从首页进子页面，手动压入历史
    // 这样用户按返回键时，popstate 才会触发，我们才能捕获并切回首页
