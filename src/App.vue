...
// --- 分享配置 ---
// 注意：图片链接已改为 https，如果QQ图床不支持https，请务必下载图片重命名为 logo.png 放入 public 文件夹
// 然后将 imgUrl 改为: 'https://yrcx.ctsfc.top/logo.png'
const SHARE_CONFIG = {
  title: '长途顺风合乘平台', // 修正为你要求的标题
  desc: '一个专注长途顺风拼车的合乘平台，老乡互助，共享出行！', // 修正为你要求的描述
  link: 'https://yrcx.ctsfc.top/', // 你的网站域名
  imgUrl: 'https://fastly.jsdelivr.net/npm/@vant/assets/cat.jpeg' // 暂时使用HTTPS演示图，请替换为你自己的图片URL
};
...
const initWxConfig = async () => {
  try {
    ...
    // 这里的 /api/wx_sign 就是你 Cloudflare Functions 提供的后端接口
    const res = await fetch(`/api/wx_sign?url=${encodeURIComponent(currentUrl)}`);
    const data = await res.json();

    if (data.appId) {
      wx.config({
        debug: false, // 调试开关：发布时设为false，测试签名失败时设为true
        appId: data.appId,
...
const getDetailShareData = (item) => {
  ...
  // 详情页分享: 标题含路线，描述含时间和广告语，不含电话
  return {
    title: `${typeStr} ${item.origin}→${item.destination}`,
    desc: `出发:${formatDate(item.date)}。备注:${cleanRemark}。 宜宾出行公众号，海量信息任你选！`,
...
const handleWeChatAuth = () => {
  ...
  setTimeout(() => {
    ...
    userProfile.wechatId = 'wx_' + Math.random().toString(36).substr(2, 8); // 自动生成微信号
