  ...
  <div style="padding-bottom: 140px; background: #f7f8fa; min-height: 100vh; position: relative;">
    ...
    <div v-show="!showTypeSelector && postForm.type" style="padding: 15px;">
        ...
        <div style="background:#fff; border-radius:12px; padding:5px 15px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); margin-bottom: 15px;">
          <van-field v-model="postForm.dateDisplay" label="出发时间" readonly is-link @click="showDate = true" label-width="80px" style="font-size: 18px; font-weight: bold;" />
          
          <div style="display: flex; align-items: center; padding: 10px 16px;">
            <div style="font-weight: bold; color: #333; width: 80px; font-size: 18px;">座位数</div>
          ...
          <van-field v-if="postForm.type==='driver'" label="车型" readonly label-width="80px" style="font-size: 18px; font-weight: bold;">
            <template #input>
          ...
          <van-field v-model="postForm.contact" type="tel" label="联系电话" placeholder="请输入手机号" label-width="80px" style="font-size: 18px; font-weight: bold;" />
          <van-field v-model="postForm.price" type="digit" label="拼车费用" placeholder="不填则为面议" label-width="80px" style="font-size: 18px; font-weight: bold;" />
        </div>

        <div style="background:#fff; border-radius:12px; padding:15px; box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
          <div style="font-weight: bold; color: #333; margin-bottom: 12px; font-size: 18px;">快捷备注标签</div>
...
const handlePublish = async () => { 
    ...
    // 🌟 核心：强制提取真正的本地登录 ID。绝不再生成 user_ 时间戳假数据！
    ...
    try { 
        ...
        // 🌟 核心拦截：如果后台返回 401（用户已被管理员删除），强制清空本地缓存，要求重登
...
.seat-box { flex-shrink: 0; width: 40px; height: 40px; border-radius: 8px; background: #f2f3f5; display: flex; align-items: center; justify-content: center; font-size: 18px; font-weight: bold; color: #666; border: 1px solid transparent; transition: all 0.2s; cursor: pointer; }
.seat-box.active { background: #1989fa; color: #fff; box-shadow: 0 4px 8px rgba(25,137,250,0.3); }

.tag { padding: 6px 14px; background: #f2f3f5; border-radius: 6px; font-size: 14px; border: 1px solid transparent; transition: all 0.15s; color:#555; cursor: pointer; }
