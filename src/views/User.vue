...
// 👉 如果新 TabBar 有报错，User 页面也可能白屏，假设新 TabBar 是好的。
...
onMounted(async () => {
    try {
        // 安全防御：确保 store 和 userProfile 存在后再发起请求
        ...
        console.error("数据加载失败，已启用最底层防崩溃处理");
    } finally {
...
const deleteRide = async (id) => {
    ...
    if (!userId) return;
    // 👉 使用浏览器原生弹窗，彻底规避组件库弹窗导致的兼容性白屏崩溃
    if (!window.confirm('此操作不可恢复，确定要删除这条行程吗？')) return;
    
    try {
        const res = await fetch(`/api/rides?id=${id}&user_id=${userId}`, { method: 'DELETE' });
        if (res.ok) {
            myRides.value = myRides.value.filter(r => r.id !== id);
            window.alert('删除成功');
        } else {
            const errData = await res.json();
            window.alert('删除失败: ' + (errData.error || '未知错误'));
        }
    } catch(e) {
        window.alert('网络请求遇到异常，无法完成删除');
    }
};
</script>
  ...
  <div class="user-page-container">
    ...
    <div class="profile-header-card">
        <template v-if="store && store.userProfile && store.userProfile.id">
            <img :src="store.userProfile.avatar || 'https://fastly.jsdelivr.net/npm/@vant/assets/cat.jpeg'" class="user-avatar" />
            <div class="user-details">
                <div class="user-nickname">{{ store.userProfile.nickname || '宜人用户' }}</div>
                <div class="user-sub-info">📱 {{ store.userProfile.phone || '未绑定手机号' }}</div>
            </div>
        </template>
        <template v-else>
    ...
    <div class="rides-section-container">
        ...
        <div v-if="isFetchingData" class="rides-loading-status">数据加载中...</div>
        
        <div v-else-if="!store || !store.userProfile || !store.userProfile.id" class="rides-empty-status">
            请登录后查看您的发布历史
        </div>
        <div v-else-if="myRides.length === 0" class="rides-empty-status">
            暂无任何发布记录
        </div>
...
.rides-loading-status, .rides-empty-status { text-align: center; padding: 40px 0; color: #999; background: #fff; border-radius: 8px; }
