                ...
                <div v-else>
                    ...
                    <div v-show="activeTab === 'users'">
                        ...
                        <div style="font-size: 13px; color: #ff6600; margin-bottom: 15px; background: #fff5eb; padding: 10px; border-radius: 6px;">
                            提示：如果您看到了以 "user_wx_" 开头的账号，这是未授权产生的错误假数据，请直接点击彻底删除。
                        </div>
                        <div v-for="u in users" :key="u.id" style="display:flex; justify-content:space-between; align-items:center; padding:15px; background: #fafafa; border-radius: 8px; margin-bottom: 10px; border: 1px solid #eee;">
                            ...
                            <div style="display:flex; gap: 8px;">
                                <button v-if="!u.is_banned || u.is_banned === 0" @click="toggleBanUser(u, 1)" style="background:#ff976a; color:#fff; border:none; padding:8px 12px; border-radius:6px; cursor: pointer; font-weight:bold;">
                                    🚫 封禁
                                </button>
...
const saveConfig = async () => {
    ...
    try {
        ...
        } else { showFailToast('部分数据保存失败'); }
    } catch (e) { showFailToast('同步异常'); }
};
...
const toggleBanUser = async (user, targetStatus) => {
    ...
    if(res.ok) { 
        ...
        showSuccessToast(targetStatus ? '已拉黑封禁' : '解封成功'); 
    }
};
