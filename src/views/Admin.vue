  ...
  <div style="background: #f0f2f5; min-height: 100vh; font-family: sans-serif; display: flex; box-sizing: border-box;">
    <div v-if="!isAdmin" style="width: 100vw; padding-top: 150px;">
        <div style="max-width: 360px; margin: 0 auto; background: #fff; padding: 40px 30px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08);">
            <div style="font-size: 20px; font-weight: bold; text-align: center; margin-bottom: 25px; color: #333;">超级管理验证</div>
                ...
                <div v-else>
                    ...
                    <div v-show="activeTab === 'ui'">
                        ...
                        <div class="form-group">
                            <label style="color:#07c160; font-weight: bold;">📢 首页滚动公告内容</label>
                        ...
                        <div style="border: 1px dashed #07c160; padding: 15px; border-radius: 8px; margin-top: 25px; background:#fafafa;">
                            <div style="display:flex; justify-content:space-between; margin-bottom:15px; align-items:center;">
                                <span style="font-weight:bold; color:#07c160;">📸 首页多图轮播</span>
                                <button @click="addBanner" style="font-size:12px; background:#07c160; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer;">+ 增加图片</button>
                            </div>
                            <div v-for="(b, idx) in bannerItems" :key="idx" style="background:#fff; padding:12px; border-radius:6px; margin-bottom:10px; border:1px solid #ddd; position:relative;">
                                <label style="font-size:12px; color:#555;">图片资源外链 (URL):</label>
                                <input v-model="b.img" class="input-ctrl" style="margin-bottom:6px;" />
                                <label style="font-size:12px; color:#555;">跳转目标地址 (选填):</label>
                    ...
                    <div v-show="activeTab === 'rides'">
                        <h3 class="section-title">全网行程记录单据管理</h3>
                        <table style="width:100%; border-collapse:collapse; font-size:14px;">
                            <tr style="background:#f4f5f6; text-align:left;">
                                ...
                                <th style="padding:12px; border:1px solid #eef0f1;">操作</th>
                            </tr>
                            <tr v-for="r in rides" :key="r.id" style="border-bottom: 1px solid #f0f0f0;">
                                ...
                                <td style="padding:12px; border:1px solid #eef0f1; display:flex; gap:8px;">
                                    <button @click="deleteRide(r.id)" style="background:#ff4d4f; color:#fff; border:none; padding:5px 12px; border-radius:4px; cursor:pointer;">强行下架</button>
                                    <button @click="quickBanUser(r.user_id)" style="background:#222; color:#fff; border:none; padding:5px 12px; border-radius:4px; cursor:pointer;">封禁发布人</button>
                                </td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <div v-show="activeTab === 'users'">
                        <h3 class="section-title">全网实名注册会员名单</h3>
                        ...
                        <h4 style="margin:25px 0 12px; color:#222; font-weight:bold; font-size:15px;">👤 会员身份对齐列表</h4>
                        <div v-for="u in users" :key="u.id" style="display:flex; justify-content:space-between; align-items:center; padding:15px; border-bottom:1px solid #f0f0f0; background: #fafafa; border-radius: 8px; margin-bottom: 8px;">
                            <div style="display:flex; align-items:center; gap:14px;">
                                ...
                                <div>
                                    ...
                                    <div style="font-size: 12px; color: #666; margin-top: 5px;">ID: {{ u.id }} | 📱 手机号: <span style="color:#ff5500; font-weight:bold;">{{ u.phone || '未留存' }}</span></div>
                                </div>
                            </div>
                            <button @click="toggleBanUser(u)" :style="{background: u.is_banned ? '#07c160' : '#ff4d4f', color:'#fff', border:'none', padding:'7px 14px', borderRadius:'6px', cursor:'pointer', fontWeight:'bold'}">
                                {{ u.is_banned ? '解除封禁' : '强制封禁' }}
                            </button>
                        </div>
                    </div>

                    <div v-show="activeTab==='base' || activeTab==='ui' || activeTab==='users'" style="border-top:1px solid #eee; margin-top:30px; padding-top:20px;">
                        <button @click="saveConfig" style="width:100%; height:46px; background:#07c160; color:#fff; border:none; border-radius:6px; font-size:16px; font-weight:bold; cursor:pointer;">保存配置生效</button>
                    </div>
                </div>
            </div>
        </div>
...
const saveConfig = async () => {
    ...
    } catch (e) { showFailToast('同步受阻'); }
};

const deleteRide = async (id) => {
    if(window.confirm('确认下架删除该记录吗？')){
        try {
            // 注意：加入了 admin=true 参数触发无条件删除
            const res = await fetch(`/api/rides?id=${id}&admin=true`, { method: 'DELETE' });
            if(res.ok) { rides.value = rides.value.filter(r => r.id !== id); showSuccessToast('行程已下架'); }
        } catch(e){}
    }
};
...
const quickBanUser = async (userId) => {
    ...
    if(window.confirm(`确认封禁并拉黑该发布人吗？`)){
        try {
            ...
            if(res.ok) { showSuccessToast('账号已封禁'); fetchAdminAssets(); }
        } catch(e){}
    }
};
...
const toggleBanUser = async (user) => {
    ...
    try {
        ...
        if(res.ok) { user.is_banned = targetStatus; showSuccessToast(targetStatus ? '已拉黑封禁' : '已解封'); }
    } catch(e){}
};
